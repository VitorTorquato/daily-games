...
// Função generateMetadata corrigida
export async function generateMetadata({ params }: PropsParams): Promise<Metadata> {
   
    const {id} = params
    try {
        const response: GameProps = await fetch(
            `${process.env.NEXT_API}/next-api/?api=game&id=${id}`,
            { next: { revalidate: 320 } }
        )
            .then((res) => res.json())
            .catch(() => {
...
// Função para buscar jogo aleatório
async function getRandomGame() {
    try {
        const response = await fetch(`${process.env.NEXT_API}/next-api/?api=game_day`, {
            cache: "no-store",
        });
...
export default async function Game({ params }: { params: { id: string } }) {
    // O params é tratado assíncronamente
    const { id } = params;
